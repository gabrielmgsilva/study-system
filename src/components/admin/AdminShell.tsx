    ...
    <div className="flex h-full flex-col bg-white text-slate-900">
      ...
      <div className="flex-1 px-3 py-5">
        <div className="space-y-1.5">
          <Link
            href={localizeAppHref(ROUTES.adminUsers, locale)}
            className="flex items-center gap-2.5 rounded-xl px-4 py-2.5 text-[13px] font-medium text-slate-600 transition hover:bg-slate-100 hover:text-slate-900"
          >
            <LayoutDashboard className="h-4 w-4" />
                  ...
                  {(['en', 'pt'] as const).map((nextLocale) => {
                    ...
                    return (
                      <Link
                        key={nextLocale}
                        href={href}
                        aria-label={`${copy.switchLanguage}: ${nextLocale.toUpperCase()}`}
                        className={[
                          'rounded-full px-3 py-1 text-xs font-semibold transition-colors',
                          isActive ? 'bg-[#2d4bb3] text-white' : 'text-slate-600 hover:bg-slate-200',
                        ].join(' ')}
                      >
